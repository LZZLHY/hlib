...
|---|---|---|---|---|
| 1000008 | 2026-06-26 | [新增] **公共下载目录设备自适应 + 授权持久化**：`PublicDownloadDir` 两条路线——平板/2in1（syscap `FolderObtain`）走 `Environment.getUserDownloadDir()` 按原始路径静默写 `下载/<包名>/`（`READ_WRITE_DOWNLOAD_DIRECTORY`）；手机（`getUserDownloadDir` 报 801）用 `DocumentViewPicker` 的 **DOWNLOAD 模式**——自动落 `下载/<应用名>/`、跳过选择界面直接返回目录 URI（无弹窗），经 `fileShare.persistPermission`（`OperationMode.READ_MODE\|WRITE_MODE`，syscap `AppFileService.FolderAuthorization` + 权限 `FILE_ACCESS_PERSIST`）持久化、缓存 URI 与路径；启动时 `activatePermission` 激活复用，之后按 `<目录>/<文件名>` 原始路径直接读写（下载落公共目录、应用内重开也可用）。下载落盘统一由 `DownloadVM.start` 解析公共目录路径并随镜像重试复用，失败回退沙箱；「导出文件」「打开下载文件夹」(`filemanager://openDirectory` 深链) 同源复用。**修正**：此前 `persistPermission` 误用 `wantConstant.Flags` 作 operationMode 导致报错，已改为 `fileShare.OperationMode`。 | [PublicDownloadDir.ets](../entry/src/main/ets/utils/PublicDownloadDir.ets) · [DownloadVM.ets](../entry/src/main/ets/viewmodel/DownloadVM.ets) · [DownloadsTab.ets](../entry/src/main/ets/tabs/DownloadsTab.ets) · [EntryAbility.ets](../entry/src/main/ets/entryability/EntryAbility.ets) · [module.json5](../entry/src/main/module.json5) | LZZLHY |
