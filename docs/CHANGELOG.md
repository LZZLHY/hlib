...
|---|---|---|---|---|
| 1000005 | 2026-06-26 | [新增] **启动静默自动检查更新 + 红点 + 24h 节流**：`UpdateChecker.autoCheck` 在 SplashPage 引导阶段触发——先把上次缓存的「有新版」状态回灌 AppStorage（即时红点、无需联网），距上次检查不足 24h 直接返回，否则联网刷新并写回缓存；首页右上齿轮与「设置 → 关于 → 检查更新」通过 `@StorageProp(UPDATE_AVAILABLE)` 显示红点。[新增] **公共下载目录持久化授权**：选择公共下载文件夹时申请 `ohos.permission.FILE_ACCESS_PERSIST`，选中后 `fileShare.persistPermission` 持久化写授权、下载写入前 `fileShare.activatePermission` 唤醒（新增 `PublicDirAccess`）；未拿到授权则不切换目录。文件夹选择仅 PC/2in1 支持，手机/平板用沙箱目录。[文档] 完善 CHANGELOG 用法与注意事项。 | [UpdateChecker.ets](../entry/src/main/ets/viewmodel/UpdateChecker.ets) · [PublicDirAccess.ets](../entry/src/main/ets/utils/PublicDirAccess.ets) · [SplashPage.ets](../entry/src/main/ets/pages/SplashPage.ets) · [HomeTab.ets](../entry/src/main/ets/tabs/HomeTab.ets) · [SettingsPage.ets](../entry/src/main/ets/pages/SettingsPage.ets) · [module.json5](../entry/src/main/module.json5) · [app.json5](../AppScope/app.json5) | LZZLHY |
