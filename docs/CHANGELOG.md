...
|---|---|---|---|---|
| 1000008 | 2026-06-26 | [新增] **公共下载目录设备自适应（手机也能写沙箱外）**：`PublicDownloadDir` 重构为两条路线——平板/2in1（syscap `FolderObtain` 支持）走 `Environment.getUserDownloadDir()` 静默写；手机（`getUserDownloadDir` 报 801）改用 `DocumentViewPicker`（`DocumentPickerMode.DOWNLOAD`）让用户**确认一次** `下载/<应用名>/` 目录，经 `fileShare.persistPermission` 持久化授权（新增 `ohos.permission.FILE_ACCESS_PERSIST`，实测可授予第三方应用、安装不被拒），目录路径与 URI 持久化到 AppStorage，下载/导出静默复用，启动时 `activatePersisted()` 重新激活。`buildFilePath` 增加可写性校验失败回退沙箱；「打开下载文件夹」改用 `filemanager://openDirectory` 深链（失败回退 `startAbility`）。`SettingsPage`/`DownloadsTab` 统一改调 `ensureDir()`。 | [PublicDownloadDir.ets](../entry/src/main/ets/utils/PublicDownloadDir.ets) · [DownloadPaths.ets](../entry/src/main/ets/viewmodel/download/DownloadPaths.ets) · [EntryAbility.ets](../entry/src/main/ets/entryability/EntryAbility.ets) · [module.json5](../entry/src/main/module.json5) | LZZLHY |
