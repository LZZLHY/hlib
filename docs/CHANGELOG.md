...
|---|---|---|---|---|
| 1000008 | 2026-06-26 | [新增] **公共下载目录设备自适应（含手机方案）**：`PublicDownloadDir` 重构为两条路线——平板/2in1（syscap `FolderObtain`）走 `Environment.getUserDownloadDir()` 按原始路径静默写 `下载/<包名>/`（user_grant `READ_WRITE_DOWNLOAD_DIRECTORY`）；手机（`getUserDownloadDir` 报 801）**实测无法按原始路径写公共目录**（`fileIo.openSync` 与 `fileShare.persistPermission` 均报 `Operation not permitted`），故手机下载一律落沙箱（可靠、不中断），改由「下载」页「导出文件」用 `DocumentViewPicker`（DOWNLOAD 模式、`newFileNames` 数组单弹窗批量授权）把文件**经返回的 URI 立即写入**公共目录（`copyPathToUri` fd 级拷贝）。`buildFilePath` 仅在平板放行公共路径；「打开下载文件夹」改用 `filemanager://openDirectory` 深链（失败回退 `startAbility`）；设置页公共目录选项仅平板展示，手机给出导出引导文案。 | [PublicDownloadDir.ets](../entry/src/main/ets/utils/PublicDownloadDir.ets) · [DownloadsTab.ets](../entry/src/main/ets/tabs/DownloadsTab.ets) · [DownloadPaths.ets](../entry/src/main/ets/viewmodel/download/DownloadPaths.ets) · [SettingsPage.ets](../entry/src/main/ets/pages/SettingsPage.ets) | LZZLHY |
