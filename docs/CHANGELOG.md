...
|---|---|---|---|---|
| 1000006 | 2026-06-26 | [修复/重构] **下载目录改为「沙箱 / 公共下载目录」二选一**：移除「缓存目录」选项；公共目录不再用 DocumentViewPicker 选文件夹（手机/平板不支持文件夹选择且方式不当），改用 `Environment.getUserDownloadDir()`（`@kit.CoreFileKit`）——文件落到**公共 Download 目录下以应用命名的专属子目录**（用户在文件管理可见）。权限由 `FILE_ACCESS_PERSIST` 换为 user_grant 的 `ohos.permission.READ_WRITE_DOWNLOAD_DIRECTORY`，切换公共目录时经 `requestPermissionsFromUser` 弹窗授权，未授权 / 不支持则回退沙箱。新增 `PublicDownloadDir`，删除 `PublicDirAccess`；`DownloadPaths` / `DownloadsTab` 同步适配。 | [PublicDownloadDir.ets](../entry/src/main/ets/utils/PublicDownloadDir.ets) · [DownloadPaths.ets](../entry/src/main/ets/viewmodel/download/DownloadPaths.ets) · [SettingsPage.ets](../entry/src/main/ets/pages/SettingsPage.ets) · [module.json5](../entry/src/main/module.json5) | LZZLHY |
