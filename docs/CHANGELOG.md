...
|---|---|---|---|---|
| 1000008 | 2026-06-26 | [新增] **公共下载目录设备自适应（手机直接下载到下载目录）**：`PublicDownloadDir` 两条路线——平板/2in1（syscap `FolderObtain`）走 `Environment.getUserDownloadDir()` 按原始路径静默写 `下载/<包名>/`（user_grant `READ_WRITE_DOWNLOAD_DIRECTORY`）；手机（`getUserDownloadDir` 报 801、原始路径写公共目录被拒）在**每次下载时**经 `DocumentViewPicker`（DOWNLOAD 模式）让用户确认一次，`save()` 返回应用专属目录 `basePath`，**在同一会话内**把文件写入 `basePath/<文件名>`（`DownloadVM.start` 选定一次目标路径、随镜像重试复用，写完落 `下载/<应用名>/`）。「下载」页「导出文件」(沙箱场景)同样用 picker `newFileNames` 数组单弹窗批量、经返回 URI 立即写入（`copyPathToUri` fd 拷贝）。「打开下载文件夹」改用 `filemanager://openDirectory` 深链（失败回退 `startAbility`）。 | [PublicDownloadDir.ets](../entry/src/main/ets/utils/PublicDownloadDir.ets) · [DownloadVM.ets](../entry/src/main/ets/viewmodel/DownloadVM.ets) · [DownloadsTab.ets](../entry/src/main/ets/tabs/DownloadsTab.ets) · [SettingsPage.ets](../entry/src/main/ets/pages/SettingsPage.ets) | LZZLHY |
